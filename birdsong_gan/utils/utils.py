...
def to_image(seq: np.ndarray, nfft: int = 256) -> np.ndarray:
    """Spectrogram computation for a sequence seq

    :param seq: input sequence
    :type seq: np.ndarray
    :param nfft: nfft, defaults to 256
...
def to_batches(segments, batch_size):
    ...
    batches = [
        np.zeros(shape=(batch_size,) + tuple(segments[0].shape))
...
def phase_restore(mag, random_phases, n_fft, N=50):
    ...
    for i in range(N):
        _, p = lc.magphase(lc.stft(lc.istft(mag * p), n_fft=n_fft))
...
def from_polar(image):
    ...
    return image[:, :, 0] * np.cos(image[:, :, 1]) + 1j * image[:, :, 0] * np.sin(
        image[:, :, 1]
    )
...
@torch.no_grad()
def overlap_encode(
    sample, netE, transform_sample=False, imageW=16, noverlap=0, cuda=True
):

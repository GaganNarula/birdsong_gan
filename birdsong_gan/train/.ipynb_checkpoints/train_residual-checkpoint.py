...
from models.nets_16col_residual import _netG_res, _netE_res, _netD, weights_init
...
#parser.add_argument('--nreslayers', type=int, default=3, help ='number of res net layers in a residual block')
...
if __name__ == '__main__':
    ...
    for epoch in range(opts_dict['niter']):
        for i, (data, age) in enumerate(train_dataloader):
            ...
            labell = torch.FloatTensor(opts_dict['batchSize'],1).fill_(0.).to(device) # false label
            err_fake_d3 = criterion_gan(pred_fake_d3,labell)
            
            labell = torch.FloatTensor(opts_dict['batchSize'],1).fill_(1.).to(device) # true label
            err_real_d3 = criterion_gan(pred_real_d3,labell)
            err_d3 = err_fake_d3+err_real_d3
            err_d3.backward()
            optimizerD3.step()

            err_g_d3 = criterion_gan(pred_fake_d3, labell)
